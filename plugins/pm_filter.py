...
async def next_page(bot, query):
    ...
    btn.insert(0, 
        [
            InlineKeyboardButton(f'📁 Fɪʟᴇ ɴᴀᴍᴇ {search} ', 'reqst1')
        ]
    )
    btn.insert(1, 
        [
            InlineKeyboardButton(f'ɪɴꜰᴏ', 'info'),
            InlineKeyboardButton(f'ᴍᴏᴠɪᴇ', 'movie'),
            InlineKeyboardButton(f'ꜱᴇʀɪᴇꜱ', 'series')
        ]
    )
    if 0 < offset <= 7:
        off_set = 0
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                ms = await client.send_cached_media(
                    chat_id=CH_FILTER,
                    file_id=file_id,
                    caption=f'<b>Hᴀɪ 👋 {query.from_user.mention}</b> 😍\n\n<code>[CLM] {title}\n\n⚙️ sɪᴢᴇ : {size}</code>\n\n⚠️ <i>Tʜɪs ғɪʟᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ғʀᴏᴍ ʜᴇʀᴇ ᴡɪᴛʜɪɴ 10 ᴍɪɴᴜᴛᴇ ᴀs ɪᴛ ʜᴀs ᴄᴏᴘʏʀɪɢʜᴛ ... !!!</i>\n\n<i>കോപ്പിറൈറ്റ് ഉള്ളതുകൊണ്ട് ഫയൽ 10 മിനിറ്റിനുള്ളിൽ ഇവിടെനിന്നും ഡിലീറ്റ് ആകുന്നതാണ് അതുകൊണ്ട് ഇവിടെ നിന്നും മറ്റെവിടെക്കെങ്കിലും മാറ്റിയതിന് ശേഷം ഡൗൺലോഡ് ചെയ്യുക!</i>\n\n<b>🧑🏻‍💻 ʀᴇǫᴜᴇsᴛᴇᴅ ʙʏ : {query.from_user.mention}\n🚀 ɢʀᴏᴜᴘ : {query.message.chat.title}</b>',
                    reply_markup = InlineKeyboardMarkup(buttons),
        ...
        buttons = [[
            ...
            InlineKeyboardButton('1 - 3', callback_data='pages'),
        ...
        buttons = [[ 
            ...
            InlineKeyboardButton('2 - 3', callback_data='pages'),
        ...
        buttons = [[ 
            ...
            InlineKeyboardButton('3 - 3', callback_data='pages'),
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        cap = TEMPLATE.format(
            query = search,
            requested = msg.from_user.mention,
